...
% Annihilation version
fprintf('\nThis demo illustrates an adaptive IIR filter in normalized\n');
fprintf('lattice form, using a simplified partial gradient algorithm\n');
fprintf('to adapt the IIR lattice filter.  This algorithm ajusts the\n');
fprintf('coefficients in an attempt to minimize the mean square\n');
...
fprintf('Figure 1 plots the poles and zeros, in the z^(-1) plane,\n');
fprintf('for the system to be identified.\n');
%
% Hard:
if eehh==2
...
fprintf('(If this is your first run, go ahead and type %d,\n',sysord);
fprintf('or hit <return>)\n');
...
% uin = randn; %zero mean, unit variance white noise, Gaussian.
 xstd(sysord+1) = uin;
...
%
 ...
 afout = nu' * x1; %adaptive filter output.
 error = xstd(sysord+1) - afout; %output error
 nu = nu + mu * error * x1/(1 + mu*x1'*x1);
 scal2 = mu*error/(1 + xpost'*xpost);
 ttvec(M+1) = 1.0;
 ttvec(1:M) = scal2*xpost(1:M);
 ttvec = lattice(ttvec,nutemp,sth,cth);
 if ttvec(1) < 0
  ttvec = -ttvec;
 end
 for kk=1:M
  tn = norm([ttvec(kk) ttvec(kk+1)]);
  if tn < (eps+eps)
   sth(kk) = 0.0;
   cth(kk) = 1.0;
  else
   sth(kk) = ttvec(kk+1)/tn;
   cth(kk) = ttvec(kk)/tn;
   ttvec(kk+1) = tn;
  end
 end
...
if M < sysord
 ...
 fprintf('If the algorithm converges to the global minimum,\n');
 fprintf('and misadjustment is adequately controlled,\n')
 fprintf('the mean square output error will be smaller than\n')
...
% clear registers
%
clear M iter nuvec sthd uin afout k pole sval x1 pval
clear cth kk rotvec syschoice xpost cthd mu scal2 sysord xstd
clear eehh nu scale t zero error nud ss tn errvec nutemp sth ttvec
% end of file.
%
